...
#' @param dir_clustr_ctrl The directory containing preprocessed control files.
#' @param marker The marker that this control corresponds to.
#' @param orig_fsom The original FlowSOM object, as generated by \code{\link[FlowSOM]{FlowSOM}}.
#' @param fsom_name 
#'
#' @details
#' Additional details...
...
#'
#' @export
prepareControls <- function(sample_df, ctrl_col, dir_clustr_ctrl, marker, orig_fsom, 
                            fsom_name = NULL, agg_name = NULL, subsetted_meta = NULL) { # agg_name unnecessary if you add metadata
  # Pre-existing clustering given
  if (!is.null(fsom_name) && !(is.null(agg_name))) { 
    print("Using the FlowSOM object defined in `fsom_name`...")
    agg_fmo <- flowCore::read.FCS(agg_name)
    fmo_fsom <- readRDS(paste0(dir_rds_edited(), fsom_name))
    
    # Is a reclustering, no pre-existing clustering given
  } else if (!is.null(subsetted_meta)) {
    frames <- lapply(list.files(dir_clustr_ctrl, full.names = TRUE), flowCore::read.FCS)

    # Get metaclusters used for the reclustering
    meta_names <- levels(orig_fsom$metaclustering) # need parent, not object data is mapped to
    ...
    names(frames) <- list.files(dir_clustr_ctrl, full.names = TRUE)
    ...
    ctrl_fsom <- FlowSOM::NewData(orig_fsom, agg_fmo)
    ...
    saveRDS(ctrl_fsom, paste0(dir_rds_edited(), ctrl_col, "_fsom.rds"))

    # Save clustered control files
    dir_save <- file.path("Data", paste0("Clustered ", ctrl_col, " Files"))
...
doDEAnalysis <- function(markers_of_interest) {
  set.seed(42)
  ...
  for (m in markers_of_interest) {
    ...
    for (k in 1:length(meta_names)) {
      ...
      if (length(rows_to_remove > 0)) { # other way to determine this?
      ...
      sample_medians <- getClusterSampleMedians()
    }
