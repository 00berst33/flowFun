...
#'
#' @param input Either a filepath to a .csv file, or a data.frame, containing sample info.
...
prepareSampleInfo <- function(input, name_col, filename_col, comparisons,
                              samples_to_remove = NULL) {
  # Check input data type
  if (is.character(input)) {
    sample_df <- utils::read.csv(file = input,
                                 header = TRUE)
  } else if (is.data.frame(input)) {
    sample_df <- input
  } else {
    stop("`input` must be a data.frame or character string.")
  }
...
#' [prepareSampleInfo()].
#' @param populations A vector of metacluster names of interest.
...
#'
#' @export
makeCountMatrix <- function(input, sample_df = NULL, populations = NULL,
...
#' @keywords internal
#' @export
makeCountMatrix.GatingSet <- function(input, sample_df = NULL, populations = NULL,
                                      min_cells = 3, min_samples = NULL) {

  # Retrieve counts from GatingSet
  gs_counts <- flowWorkspace::gs_pop_get_count_fast(input, format = "wide", path = "auto")

  # Get row indices of populations of interest
  keep_idx <- rownames(gs_counts) %in% populations

  # Subset matrix
  gs_counts <- gs_counts[keep_idx, ]
...
#' @keywords internal
#' @export
makeCountMatrix.FlowSOM <- function(input, sample_df, populations = NULL,
                                    min_cells = 3, min_samples = NULL) {
  if (nrow(sample_df) > 0) {
    # Set `populations` if it is NULL
    if (is.null(populations)) {
      populations <- levels(input$metaclustering)
    }
...
#' @keywords internal
#' @export
makeCountMatrix.data.frame <- function(input, sample_df = NULL, populations = NULL,
                                       min_cells = 3, min_samples = NULL) {
  ...
  counts <- input %>%
    tidytable::filter(Metacluster %in% populations) %>%
...
#' \code{\link[edgeR:calcNormFactors]{calcNormFactors()}}, one of `"TMM",
#' "TMMwsp", "RLE", "upperquartile", or "none"`. Default is `"TMM"`. See documentation
#' of `calcNormFactors()` for more details.
#'
#' @details
...
#' likelihood ratio tests using functions from edgeR.
#'
#' @return A list of tables extracted from a
#' \code{\link[edgeR:`TopTags-class`]{TopTags}} object. Each element
#' corresponds to a column in `contrasts`.
#'
#' @seealso \code{\link[edgeR:calcNormFactors]{calcNormFactors()}},
#' \code{\link[edgeR:DGEList]{DGEList()}}, \code{\link[edgeR:estimateDisp]{estimateDisp()}},
#' \code{\link[edgeR:glmFit]{glmFit()}}, \code{\link[edgeR:glmLRT]{glmLRT()}},
#' \code{\link[edgeR:topTags-class]{topTags()}}.
#'
#' @export
doDAAnalysis <- function(design, counts, contrasts, sample_df,
  ...
  for (i in 1:ncol(contrasts)) {
    ...
    top <- edgeR::topTags(glm_lrt, n = 100, adjust.method = "BH", sort.by = "PValue")
...
gs_makeMFIMatrix <- function(gs, cols, subpopulations, inverse = FALSE) {
  ...
  mfis <- mfis %>%
    ...
    dplyr::mutate(feature = paste0(pop, ".", feature), .keep = "unused")
...
gs_makeDeltaMFIs <- function(gs, control_gs, subpopulations, cols, metadata_col = NULL) {
  ...
  if (!is.null(metadata_col)) {
    ...
    ctrl_match <- unique(ctrl_match) # for case that a few controls are used for multiple samples
...
#' @param contrasts A contrasts matrix as generated via [makeContrastsMatrix()]
#' @param counts The counts matrix as generated by [makeCountMatrix()]. If given,
#' it will be used to calculate weights for each cluster-marker being tested
...
#' @param print_table Boolean, whether to print results as formatted table.
#' Default is `TRUE`. If `FALSE`, the default output from [limma::eBayes()] is returned
#'
...
doDEAnalysis <- function(input, cols_to_test, design, contrasts, counts = NULL,
  ...
  if (is(input, "data.frame")) {
    if (inverse) {
      stop("Setting `inverse=TRUE` is only valid for GatingSets!")
    }
  ...
  weights <- NULL
  # If counts matrix is given
  if (!is.null(counts)) {
    # create weights
    total_counts <- rowSums(counts)

    # need to order and repeat according to expression matrix
    # capture everything before period to get unique cluster names
    features <- collapsed %>%
      dplyr::mutate(feature = sub("^([^.]+).*", "\\1", feature)) %>%
      dplyr::pull(feature) %>%
      unique()

    # match order of `total_counts` to order of expression matrix
    match_idx <- match(features, names(total_counts))
    if (any(is.na(match_idx))) {
      stop("Unable to match feature names. Check that each element in `subpopulations` match a rowname in `counts`.")
    }
    weights <- total_counts[match_idx] %>%
      rep(each = length(cols_to_test)) # then repeat with `rep(each = x)`
    # output: vector of weights with length equal to `nrow` of the expression matrix (`collapsed`)
  }
  ...
  all_res <- lapply(seq_len(ncol(contrasts)), function(i) {
    ...
    res <- limma::eBayes(contrasts_fit, trend = TRUE)

    # Return results as a summarized table, if desired (default)
    if (print_table) {
      res <- limma::topTable(res, sort.by = "P") %>%
        dplyr::select(feature, AveExpr, P.Value, adj.P.Val)
    }

    return(res)
  })
  names(all_res) <- colnames(contrasts)

  return(all_res)
}

fitContrasts <- function(lm_model, contrast, print_table) {
  # Perform statistical tests.
  contrasts_fit <- limma::contrasts.fit(lm_model, contrast)
  res <- limma::eBayes(contrasts_fit, trend = TRUE)
...
#' @param col Character, the name of the marker/channel to create the plot for.
