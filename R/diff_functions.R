...
#'
#' @export
clusterControls <- function(fsom, subsetted_meta = NULL, ...) {
...
clusterControls.FlowSOM <- function(fsom, sample_df, ctrl_col, dir_prepr_ctrl, marker,
...
doDEAnalysis.data.frame <- function(input, sample_df, design, contrasts, counts,
  ...
  for (m in markers_of_interest) {
    ...
    markers <- sapply(colnames(input), gsub, pattern = ".*<(.*?)>.*", replacement = "\\1")
    channel <- names(which(markers == m))
    # channel <- NULL
    # fcs_cols <- attr(input, "cols_from_fcs")
    ...
    for (k in 1:length(meta_names)) {
      ...
      #row_matches <- match(rownames(sample_medians), sample_df$File.Name)###
      #rownames(sample_medians) <- sample_df[row_matches, "Sample.Name"]###
      ...
      if (!is.null(transform)) {
        ...
        if (m %in% names(ctrl_list)) {
          ...
          ctrl_dt_sub <- ctrl_dt %>%
            tidytable::filter(Metacluster == meta_names[k])  %>%
            tidytable::select(-c("Metacluster", "Meta_original", "Cluster"))# ensure that excluded samples aren't analyzed
          ...
          # Get cluster-sample medians
          fmo_sample_medians <- getSampleMFIsTable(trans_ctrl_dt_sub)
