...
#'
#' @param input A FlowSOM object or data table, to be used for differential
#' expression analysis.
...
#'
#' @export
clusterControls <- function(input, subsetted_meta = NULL, ...) {
  result <- UseMethod("clusterControls")
  return(result)
}

#' clusterControls.FlowSOM
#'
#' Prepare isotype or FMO controls for calculating delta MFI.
#'
#' @keywords internal
#' @param input A FlowSOM object, to be used for differential expression analysis.
...
#' @param parent_ctrl_input The parent control FlowSOM whose cells are to be
#' reclustered, if \code{fsom} is a reclustering. Default is \code{NULL}.
#'
#' @return A FlowSOM object.
#'
#' @export
clusterControls.FlowSOM <- function(input, sample_df, ctrl_col, dir_prepr_ctrl, marker,
                            subsetted_meta = NULL, dir_clustr_ctrl = NULL,
                            parent_ctrl_input = NULL
                            ) {
  ...
  if (!is.null(subsetted_meta)) {
    ...
    meta_names <- levels(parent_ctrl_input$metaclustering)
    meta_ind <- which(meta_names %in% subsetted_meta)

    # Create flowSet of cells only belonging to given metaclusters
    frames <- lapply(frames, filterData, metaclusters = meta_ind)
    ...
    ctrl_fsom <- FlowSOM::NewData(input, agg_ctrl)
...
#'
#' @export
clusterControls.data.frame <- function(input, fsom, subsetted_fsom = NULL, subsetted_meta = NULL,
...
getSampleMFIs.data.frame <- function(input) {
  ...
  cols_to_use <- attr(input, "cols_from_fcs")
  ...
  medians <- input %>%
    ...
    tidytable::summarise(tidytable::across(.cols = cols_to_use,
                                           .fns = stats::median,
                                           .drop = "keep")) %>%
    tidytable::select(c(.id, cols_to_use)) %>%
...
getSampleMetaclusterMFIsHelper <- function(input, sample_df, marker_of_interest,
  ...
  for (i in seq_along(fcs_cols)) {
    ...
    if (!is.null(attr(input[[col]], "marker"))) {
      if (!is.na(attr(input[[col]], "marker")))
        if (attr(input[[col]], "marker") == marker_of_interest) {
          channel <- col
          break
        }
    }
  ...
  if (is.null(channel)) {
    stop(paste0("No corresponding channel was found for marker ", marker_of_interest,
                " check parameter `markers_of_interest` for typos."))
  }

  # For each metacluster of interest
  for (k in 1:length(meta_names)) {
    # Get cells belonging to each sample in current metacluster
    input_sub <- input %>%
      tidytable::filter(Metacluster == meta_names[k]) %>%
      tidytable::select(-c("Metacluster", "Meta_original", "Cluster"))
    ...
    if (!is.null(transform)) {
      ...
      trans_sample_medians <- flowCore::flowFrame(as.matrix(sample_medians, rownames = TRUE)) # loses attributes, consider copying transformed values onto old table?
      trans_sample_medians <- flowCore::transform(trans_sample_medians, inv)
      trans_sample_medians <- flowCore::exprs(trans_sample_medians)

      # Subtract FMO or ISO if necessary
      if (!is.null(ctrl_input)) {
        print("Preparing controls...")

        print(ctrl_input)
        # if (methods::is(ctrl_input, "data.table")) {
        #   ctrl_fsom <- clusterControls(ctrl_dt,
        #                                fsom = fsom,
        #                                subsetted_meta = subsetted_meta)
        # } else {
        #   ctrl_dt <- doPreprocessing(ctrl_input,
        #                              ld_channel = attr(input, "gating_scheme")$live_gate$channels[1],
        #                              compensation = attr(input, "compensation_matrix"),
        #                              transformation = attr(input, "transformation"),
        #                              transformation_type = "logicle", ###
        #                              debris_gate = attr(input, "gating_scheme")$debris_gate,
        #                              live_gate = attr(input, "gating_scheme")$live_gate,
        #                              # nmad = ,
        #                              pctg_live = 0,
        #                              pctg_qc = 0)
        #   ctrl_fsom <- clusterControls(ctrl_dt,
        ...
        ctrl_dt_sub <- ctrl_fsom %>%
          tidytable::filter(Metacluster == meta_names[k])  %>%
          tidytable::select(-c("Metacluster", "Cluster"))# ensure that excluded samples aren't analyzed
      ...
      # Get vector of transformed medians for current marker
      trans_matrix <- trans_sample_medians[, channel]
    }
    ...
    # Get vector of medians for current marker
    expr_matrix <- sample_medians[, channel]

    # Make data frame to store info about this metacluster.
    df <- data.frame(expr_matrix,
                     "Transformed" = trans_matrix,
                     sample = sample_df[which(rownames(sample_medians) %in% sample_df[, 2]), 1],
                     cell_type = meta_names[k],
                     check.names = FALSE)
  ...
  colnames(df_full)[1] <- marker_of_interest
...
doDEAnalysis.data.frame <- function(input, sample_df, design, contrasts, counts,
                                    markers_of_interest, meta_names = NULL,
                                    ctrl_list = NULL, subsetted_meta = NULL) {
  Metacluster <- .id <- NULL
  ...
  for (m in markers_of_interest) {
    ...
    for (i in seq_along(fcs_cols)) {
      ...
      if (!is.null(attr(input[[col]], "marker"))) {
        if (!is.na(attr(input[[col]], "marker")))
          if (attr(input[[col]], "marker") == m) { # change so there are no NAs in preprocessing
            channel <- col
            break
          }
      }
