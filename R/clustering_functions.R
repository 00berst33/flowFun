...
#'
#' @keywords internal
#' @return A data.table or flowFrame, where only cells belonging to the given clusters or
#' metaclusters are included.
#'
#' @export
filterData <- function(input, clusters = NULL, metaclusters = NULL) {
  results <- UseMethod("filterData")
  return(results)
}
...
#'
#' @keywords internal
#' @return A tidytable, where only cells belonging to the given clusters or
#' metaclusters are included.
#'
#' @export
...
#'
#' @export
filterData.flowFrame <- function(input, clusters = NULL, metaclusters = NULL) {
  filter_clust <- flowCore::exprs(input)[,"FlowSOM_cluster"] %in% clusters
  filter_meta <- flowCore::exprs(input)[,"FlowSOM_meta"] %in% metaclusters
...
#'
#' @return An aggregated \code{\link[flowCore]{flowFrame}} or data table, where only cells
...
createFilteredAggregate.FlowSOM <- function(input, num_cells, clusters = NULL,
  ...
  filtered_frames <- lapply(frames, filterData, clusters, metaclusters)
...
#' to channels.
#'
#' @param input The aggregate flowFrame or data table used to generate the given FlowSOM object.
...
revertPCAFlowSOM <- function(input, fsom, pca_obj, num_components) {
  result <- UseMethod("revertPCAFlowSOM")
  return(result)
}

#' @keywords internal
#' @export
revertPCAFlowSOM.flowFrame <- function(input, fsom, pca_obj, num_components) {
  fsom$data <- flowCore::exprs(input)
  mcl <- node <- NULL

  # metaclusterMFIs
  fsom$map$metaclusterMFIs <- data.frame(flowCore::exprs(input),
  ...
  # prettyColnames
  all_markers <- Biobase::pData(flowCore::parameters(input))$desc
  all_channels <- Biobase::pData(flowCore::parameters(input))$name
  marker_df <- stats::na.omit(data.frame(marker = all_markers,
                                         channel = all_channels))
  rownames(marker_df) <- NULL

  agg_colnames <- colnames(flowCore::exprs(input))
...
revertPCAFlowSOM.data.frame <- function(input, fsom, pca_obj, num_components) {
  ...
  fsom$map$metaclusterMFIs <- input %>%
    tidytable::mutate(Metacluster = FlowSOM::GetMetaclusters(fsom)) %>%
    getMetaclusterMFIs(cols_to_use = colnames(fsom$data))
...
#' @param convert_to_channels Logical, should the returned/saved FlowSOM object's data
...
#'
#' @return A FlowSOM object or data table, depending on the data type of \code{input}.
#'
#' @export
clusterSubsetWithPCA <- function(input, pca_obj, num_components,
                                 num_clus, xdim = 10, ydim = 10, seed = NULL,
                                 convert_to_channels = TRUE, fsom_file = NULL) {
...
clusterSubsetWithPCA.FlowSOM <- function(input, pca_obj, num_components,
  ...
  if (convert_to_channels) {
    fsom_subset <- revertPCAFlowSOM(input, fsom_subset, pca_obj, num_components)
  }

  # Save original FlowSOM object if desired
  if (!is.null(fsom_file)) {
    saveRDS(fsom_subset, file.path(dir_rds_unedited(), fsom_file))
  }
