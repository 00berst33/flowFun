...
- name_col:
  ...
  The name of the column containing the experiment's sample names.

- filename_col:

  The name of the column containing the experiment's .fcs file names.
...
- samples_to_remove:
  ...
  An integer vector defining the rows of any samples that should be
  excluded from the analysis, either because they were discarded during
  preprocessing or are not of interest. Default is none (`NULL`).
...
If you have a .csv file specifying sample information, it can be read in
and prepared by this function. It MUST have columns for file names and
sample names, and any additional columns should specify parameters of
interest, like NAC vs. No NAC. Each column should be named after what it
represents. This file will be read in as a data frame and will be used
...
#> Error: object 'sample_df' not found
...
#> Error: object 'sample_df.rm' not found
```
