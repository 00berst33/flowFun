...
# saveRDS(trans, "RDS/iso_logicle_transformation.rds")
...
fc = suppressWarnings(flowCut(f = ff_l,
             FileID = ref_filename,
             Plot = Plot,
...
# analysis.
pdf(paste0(dir_prepr_res, "human_iso.pdf"))
for (file in raw_files) {
  print(file)
  ff = read.FCS(paste0(dir_raw, file), truncate_max_range = FALSE)
  
  # pData(parameters(ff))$desc[29] = "CD21"
