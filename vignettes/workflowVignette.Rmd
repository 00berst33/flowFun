...
It is also necessary for the user to specify which data to use for clustering. There are a few considerations to account for when making this choice, the most important being that it is highly recommended that any markers the user intends to test for differential expression are NOT used for clustering. There is a number of reasons for this, but in short, it generally makes little sense to test for differential expression of a marker in a cluster when the cluster is defined by high or low expression of said marker (i.e. testing for differential expression of CD8 in CD8 T cells). Furthermore, parameters like side and forward scatter should not be included, and if a viability stain was used, it should also be excluded.

Another important consideration the user should take into account is that, even excluding those described above, not all markers in an experiment's panel are relevant to or appropriate for a clustering; for instance, certain markers tell you more about a cell's state than its phenotype. The reason for this concern becomes more clear when examining a specific cell population - in high-parameter cytometry especially, it is likely that the experiment's panel will contain a diverse set of markers meant to detect many different types of cells. Say the user decides to zoom in on a particular cell type, such as monocytes, via either reclustering or gating. Markers specific to other cell populations like dendritic cells, B cells, T cells, macrophages, etc. become either entirely irrelevant or significantly less informative. 

In order to provide a solution for this, the user is given the option to perform PCA on their data and choose the most relevant principal components for clustering, instead of manually selecting whatever markers they feel are most appropriate. (Note that any parameters that are certainly not relevant for clustering, like the viability stain or those intended to be tested for differential expression, should still be excluded before performing PCA.) This is greatly encouraged when doing any reclustering, but may also be used for any initial clustering. This approach is further explained and demonstrated in the next section, but for now the workflow will be demonstrated without PCA.
...
```{r cluster-heatmap, fig.height=5, fig.width=5, fig.align='center'}
