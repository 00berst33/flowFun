...
The first is to adjust the `gatingTemplate`. In many cases, restricting the range in which the gate can be drawn resolves this kind of issue. To set a minimum and maximum value for a gate drawn by `gate_mindensity`, edit the argument `gate_range`. The code below demonstrates this with the `data.table` created earlier, and plots the new gates generated after applying the new gating scheme.
...
Finally, we may also adjust a gate by manually redrawing it. This can be done through use of the optional `CytoExploreR` package and its function `cyto_gate_edit()`. A window where the user may draw a new gate on the sample of interest is then brought up. Once the gate has been drawn the data in the underlying `GatingSet` is edited accordingly. An example is shown below, but not run. Note that the `gatingTemplate` must be saved to a CSV file to run this function.
