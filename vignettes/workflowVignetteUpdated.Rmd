...
For instance, note the gate defined by the first row of `gt_table`, whose alias is `nonMargins`. In the `gating_method` column, we see that the value is `"boundary"`. We have `gating_args="min=c(0,0),max=c(262143,262143)"`. As a result, the gate cannot be drawn outside of the interval [0, 262143] for FSC-A or SSC-A.

The user will have to supply a compensation matrix and choose whatever transformation they would like to apply. Additionally, if your panel includes a viability stain, it is necessary to specify the corresponding channel, so that dead cells may be gated out accurately.
...
```{r umap-print, echo=FALSE, out.width = '70%', fig.align='center'}
fsom_over <- overwriteMetaclusterNames(fsom_dt, fsom_sub)
...
Before testing, median fluorescent intensities (MFIs) must be found for the markers of interest. If expression data is in a `data.table` format, the function `getExprMatDE()` may be used to do so. If using a `GatingSet` object, the function `flowWorkspace::gs_pop_get_stats()` with argument `type = pop.MFI` should be used, with some edits to prepare it to be passed to `limma::lmFit()`. 
...
var <- rlang::enquo(marker_cols)

# Get MFIs from GatingSet
mfis <- gs_pop_get_stats(gs1, nodes = subpops, type = pop.MFI) # may set inverse_transform = TRUE for raw data scale
mfis <- mfis %>% 
