...
`FlowSOM()` does not take `GatingSet` data structures as input, so to use this function users should subset their data to the appropriate population with `gs_pop_get_data()` and pass the result to `cytosetToTable()`. This table may then be passed to `flowSOMWrapper()`.

``` {r flowsom-clustering}
# Read in table
# prepr_table <- system.file("extdata", "prepr_table.rds", package = "flowFunData")
# prepr_table <- readRDS(prepr_table)
# prepr_table <- prepr_table %>% 
#   dplyr::select(-cell_id) # edit

fs <- flowWorkspace::gs_pop_get_data(gs1, y = "live")
fs <- flowWorkspace::cytoset_to_flowSet(fs)
sample_names <- flowCore::sampleNames(fs)
fs <- lapply(seq_along(fs), function(i) {
  ff <- fs[[i]]
  ff@description$`$FIL` <- sample_names[[i]]
  return(ff)})
fs <- as(fs, "flowSet")
...
fsom_over <- overwriteMetaclusterNames(fsom_dt, fsom_sub)
knitr::include_graphics("images/workflow_basic/umap_3.png")
```
...
knitr::include_graphics("images/workflow_basic/group_umap1.png")
knitr::include_graphics("images/workflow_basic/group_umap2.png")
knitr::include_graphics("images/workflow_basic/parent_umap.png")
```
