...
There are four data types crucial to this pipeline that users should familiarize themselves with. The following tables summarizes them:
...
Working with a `GatingSet` is quite similar to the examples above. The main differences come up when importing data. The chunk below gives an example of how a set of FCS files may be read into a `GatingSet`.

```{r import-gatingset-from-fcs}
...
Instead of a call to `read.flowSet()`, we use `load_cytoset_from_fcs()`, then `GatingSet()`. 
